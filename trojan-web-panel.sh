...
if test -s /etc/nginx/nginx.conf; then
  ...
  wget -P /etc/nginx https://raw.githubusercontent.com/ssrv2ray/Trojan_panel_web/master/nginx.conf
	green "================================="
	blue "     请输入Trojan绑定的域名"
	green "================================="
	read your_domain
  sed -i "s/localhost/$your_domain/;" /etc/nginx/nginx.conf
	...
	wget https://github.com/ssrv2ray/Trojan/raw/master/web.zip
	unzip web.zip
	green " "
	green "================================="
	blue "       开始配置trojan-web"
  ...
  green " "
	...
	green " "
	 ...
	 blue "  https://www.ssrv2ray.com/trojanpanel.html "
	green " "
...
start_menu(){
  clear
   ...
   blue " 本脚本支持：Debian9+ / Ubuntu16.04+ / Centos7+"
	 blue " 网站：www.ssrv2ray.com （已开启禁止国内访问）"
	 blue " "
	 blue " "
	green "=========================================================="
